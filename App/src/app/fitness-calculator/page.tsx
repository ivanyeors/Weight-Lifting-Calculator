...
import { FitnessCalculatorSidebar } from "./fitness-calculator-sidebar"
import { 
  loadAllExerciseData,
...
export default function HomePage() {
  ...
  // Sidebar collapsed state (moved toggle to header)
  const [sidebarCollapsed, setSidebarCollapsed] = useState<boolean>(false)
  
  // Sync status state (for display in header)
  ...
  // Monitor sync status from Supabase auth and localStorage
  useEffect(() => {
    const checkSyncStatus = async () => {
      try {
        ...
        const storedPlan = typeof window !== 'undefined'
          ...
          : null
        const plan = metaPlan || storedPlan || 'Free'
        setCurrentTier(plan)
        
        if (plan === 'Personal' && data.session?.user?.user_metadata?.fitspo_personal) {
          const saved = data.session.user.user_metadata.fitspo_personal as { updatedAt?: string }
          if (saved?.updatedAt) {
    ...
    <AuthCallbackHandler>
      <div className="flex h-screen">
        <FitnessCalculatorSidebar
          ...
          onLastSyncTimeChange={setLastSyncTime}
        />
        {!sidebarCollapsed && (
          <div
